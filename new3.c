...
void main(void)
{
	...
	int *comma;	// initializing the array to store the position of commas in the set command
...
void command_config(int *comma){
	...
	char str1[3];	// initializing arrays to store number characters between the commas
	char str2[3];
	char str3[3];

	for (pointer = 0; pointer < char_cnt; pointer++){

		if (command[pointer] == 'A' && command[pointer + 1] == 'C'){	// clear false commands
		...
		else if (command[pointer] == 'R' && command[pointer + 1] == 'S'){	// clear LED matrix
	...
	if (command[char_cnt - 1] == ';'){
	    ...
	    if (char_cnt == 2){
	    	...
	    	int pix,green,blue,red;	// a wrong place for initialization?
	    	...
	    	if( char_cnt > 10 && char_cnt < 19){
	    		...
	    		for (pin = 6; pin < char_cnt; pin++){
	    			...
	    			if (command[pin] == ','){
	    				...
	    				comma[q] = pin;	// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<This is the core of the problem>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
	    				q = q + 1;
	    			}
	    		}

	    		int comma1 = comma[0];	// next step, pick out numbers between commas and convert them to integers
	    		int comma2 = comma[1];
	    		...
	    		for (;;){
	    			...
	    			if (char_cnt > 0)	// if another command is typed in
	    				break;
	    		}
	    	}
	    }
